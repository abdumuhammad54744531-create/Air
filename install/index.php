...
$isLocalHost = $host === '' || preg_match('/^(localhost|127\.0\.0\.1|::1)(:\d+)?$/i', $host);
$scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$defaultUrl = $host !== '' ? $scheme.'://'.$host : 'http://aplikasi-web-air.test';
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    verify_csrf();
    try {
        $environment = ($_POST['app_env'] ?? $defaultEnvironment) === 'production' ? 'production' : 'local';
        $env = [
            'APP_NAME'=>$_POST['app_name'] ?? 'Sistem Informasi Monitoring dan Manajemen Air',
            'APP_ENV'=>$environment,'APP_DEBUG'=>$environment === 'production' ? 'false' : 'true','APP_URL'=>rtrim($_POST['app_url'] ?? $defaultUrl, '/'),
            ...
            'PUBLIC_PAGE_ENABLED'=>'true','DEVICE_OFFLINE_MINUTES'=>'15','DASHBOARD_REFRESH_SECONDS'=>'30','SESSION_TIMEOUT_MINUTES'=>'120','APP_ALLOW_REINSTALL'=>'false'
        ];
        $contents='';
...
<body><main class="container"><section class="card install-card"><div class="brand p-4"><h1 class="h3 mb-1">Instalasi Sistem Monitoring Air</h1><p class="mb-0 opacity-75">Konfigurasi untuk lokal maupun domain/hosting</p></div><div class="card-body p-4 p-md-5">
...
<div class="col-md-2"><label class="form-label">Lingkungan</label><select class="form-select" name="app_env"><option value="local" <?=$defaultEnvironment==='local'?'selected':''?>>Lokal</option><option value="production" <?=$defaultEnvironment==='production'?'selected':''?>>Domain</option></select></div>
<div class="col-md-3"><label class="form-label">URL aplikasi</label><input class="form-control" name="app_url" value="<?=e($defaultUrl)?>" required></div>
