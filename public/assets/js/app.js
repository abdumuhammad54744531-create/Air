document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(networkBoard){
    ...
    const showInspector=node=>{
      ...
      const metrics=node.type==='source'
        ...
        :node.type==='reservoir'
          ...
          :node.type==='service_area'
            ?[['Penduduk',new Intl.NumberFormat('id-ID').format(node.population||0)],['Kebutuhan puncak',`${formatWaterNumber(node.demand)} L/s`],['Elevasi',`${formatWaterNumber(node.elevation)} m`],['Prioritas',(node.priority||'').replaceAll('_',' ')],['Jenis','Wilayah layanan']]
      ...
      const visualType=node.master_type||node.type,inspectorIcon=visualType==='node'?(nodeKindIcon[node.node_kind]||'bi-circle-fill'):visualType==='source'?'bi-droplet-fill':visualType==='reservoir'?'bi-box-fill':'bi-houses-fill';
      inspector.innerHTML=`<div class="inspector-head"><span class="node-icon ${escapeHtml(node.type.replaceAll('_','-'))} kind-${escapeHtml(visualType.replaceAll('_','-'))}"><i class="bi ${inspectorIcon}"></i></span><div><small>${escapeHtml(typeLabel[visualType]||typeLabel[node.type])}</small><h3>${escapeHtml(node.name)}</h3><p>${escapeHtml(node.code)}</p></div></div><dl class="inspector-metrics">${metrics.map(metric=>`<div><dt>${escapeHtml(metric[0])}</dt><dd>${escapeHtml(metric[1])}</dd></div>`).join('')}</dl><div class="inspector-actions">${editAction}<button class="btn btn-primary w-100" type="button" id="inspectorConnect"><i class="bi bi-bezier2"></i> Tambah Pipa dari Titik Ini</button></div><small class="inspector-help"><i class="bi bi-bezier2"></i> Satu titik dapat terhubung dengan banyak pipa tanpa batas.</small>`;
    ...
    nodeKindSelect.addEventListener('change',()=>{
      ...
      if(['source','reservoir'].includes(kind)){
        ...
        const originalKind=nodeForm.dataset.originalKind||'';
        if(head&&(head.value===''||!['source','reservoir'].includes(originalKind))){
    ...
    const openNode=node=>{
      nodeForm.reset();nodeForm.dataset.originalKind=node.node_kind||'junction';document.querySelector('#networkNodeMethod').value='';document.querySelector('#networkNodeId').value=node.id;
